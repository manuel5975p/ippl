...
namespace ippl {
    ...
    FDTDSolver<Tfields, Dim, M, C>::FDTDSolver(Field_t& charge, VField_t& current, VField_t& E,
                                               VField_t& B, size_t pcount,
                                               LorentzFrame<Tfields> frameBoost_,
                                               undulator_parameters<Tfields> up,
                                               FDTDBoundaryCondition bcond,
                                               FDTDParticleUpdateRule pur, FDTDFieldUpdateRule fur,
                                               double timestep, bool seed_, VField_t* radiation)
        : radiation_mp(radiation)
        , bconds_m(bcond)
        , particle_update_m(pur)
        , field_update_m(fur)
        , pcount_m(pcount)
        , pl(charge.getLayout(), charge.get_mesh())
        , bunch(pl)
        , tracer_bunch(pl)
        , frameBoost(frameBoost_)
        , uparams(up) {
        absorbed__energy = 0.0;
        ...
        auto hr              = this->hr_m;
        scalar dt            = this->dt;
        auto bcsetter_single = [&scalar_bcs, &vector_bcs, hr, dt,
                                bcond]<size_t Idx>(const std::index_sequence<Idx>&) {
    ...
    void FDTDSolver<Tfields, Dim, M, C>::fill_initialcondition(callable c) {
        ...
        Kokkos::parallel_for(
            "Assign sinusoidal source at center", ippl::getRangePolicy(view_a, 0),
            KOKKOS_LAMBDA(const int i, const int j, const int k) {
                const int ig = i + ldom[0].first() - nghost;
                const int jg = j + ldom[1].first() - nghost;
                const int kg = k + ldom[2].first() - nghost;

                Tfields x         = (Tfields(ig) + 0.5) * hr[0];  // + origin[0];
                Tfields y         = (Tfields(jg) + 0.5) * hr[1];  // + origin[1];
                Tfields z         = (Tfields(kg) + 0.5) * hr[2];  // + origin[2];
    ...
    void FDTDSolver<Tfields, Dim, M, C>::solve() {
        ...
        if (field_update_m == FDTDFieldUpdateRule::DO) {
            ...
            Kokkos::parallel_for(
                ...
                KOKKOS_LAMBDA(const size_t i, const size_t j, const size_t k) {
                    ...
                    // interior values
                    bool isInterior =
                        true;  //((ig > 0) && (jg > 0) && (kg > 0) && (ig < nr_m[0] - 1)
                    ...
                    view_phiNp1(i, j, k) =
                        isInterior * interior + !isInterior * view_phiNp1(i, j, k);
                });
                ...
                Kokkos::parallel_for(
                    ...
                    KOKKOS_LAMBDA(const size_t i, const size_t j, const size_t k) {
                        ...
                        bool isInterior =
                            true;  //((ig > 0) && (jg > 0) && (kg > 0) && (ig < nr_m[0] - 1)
                                   //&& (jg < nr_m[1] - 1) && (kg < nr_m[2] - 1));
                        // if(view_JN(i, j, k)[gd] != 0)
                        //     LOG("nonzero js: " << view_JN(i, j, k)[gd]);
                        double interior =
                            -view_aNm1(i, j, k)[gd] + a1 * view_aN(i, j, k)[gd]
            ...
            first_order_abc<scalar, 0, 1, 2> abc_x[] = {
                first_order_abc<scalar, 0, 1, 2>(hr_m, c, dt, 1),
                first_order_abc<scalar, 0, 1, 2>(hr_m, c, dt, -1)};
            first_order_abc<scalar, 1, 0, 2> abc_y[] = {
                first_order_abc<scalar, 1, 0, 2>(hr_m, c, dt, 1),
                first_order_abc<scalar, 1, 0, 2>(hr_m, c, dt, -1)};
            first_order_abc<scalar, 2, 0, 1> abc_z[] = {
                first_order_abc<scalar, 2, 0, 1>(hr_m, c, dt, 1),
                first_order_abc<scalar, 2, 0, 1>(hr_m, c, dt, -1)};
            ippl::Vector<size_t, 3> extenz = ippl::Vector<size_t, 3>{
                (size_t)nr_m[0] + nghost_a * 2, (size_t)nr_m[1] + nghost_a * 2,
                (size_t)nr_m[2] + nghost_a * 2};
            for (unsigned _d = 0; _d < Dim; _d++) {
                if (bconds_m[_d] == FDTDBoundaryCondition::ABC_MUR) {
                    Kokkos::parallel_for(
                        ...
                        KOKKOS_LAMBDA(const size_t i, const size_t j, const size_t k) {
                            ...
                            ippl::Vector<axis_aligned_occlusion, 3> bocc = boundary_occlusion_of(
                                0, ippl::Vector<size_t, 3>{(size_t)ig, (size_t)jg, (size_t)kg},
                                extenz);
                            ...
                            // for (unsigned _d = 0; _d < Dim; _d++)
                            {
                                if (+bocc[_d]) {
                                    ...
                                    if (_d == 0) {
                                        view_phiNp1(i, j, k) =
                                            view_phiN(i, j, k)
                                            + (view_phiN(i + offs, j, k) - view_phiN(i, j, k)) * dt
                                                  / hr_m[0];
                                    }
                                    if (_d == 1) {
                                        view_phiNp1(i, j, k) =
                                            view_phiN(i, j, k)
                                            + (view_phiN(i, j + offs, k) - view_phiN(i, j, k)) * dt
                                                  / hr_m[1];
                                    }
                                    if (_d == 2) {
                                        view_phiNp1(i, j, k) =
                                            view_phiN(i, j, k)
                                            + (view_phiN(i, j, k + offs) - view_phiN(i, j, k)) * dt
                                                  / hr_m[2];
                                    }
                                }
                            }
                            ...
                            // for (unsigned _d = 0; _d < Dim; _d++)
                            {
                                if (+bocc[_d]) {
                                    ...
                                    if (_d == 0) {
                                        view_aNp1(i, j, k) =
                                            view_aN(i, j, k)
                                            + (view_aN(i + offs, j, k) - view_aN(i, j, k)) * dt
                                                  / hr_m[0];
                                    }
                                    if (_d == 1) {
                                        view_aNp1(i, j, k) =
                                            view_aN(i, j, k)
                                            + (view_aN(i, j + offs, k) - view_aN(i, j, k)) * dt
                                                  / hr_m[1];
                                    }
                                    if (_d == 2) {
                                        view_aNp1(i, j, k) =
                                            view_aN(i, j, k)
                                            + (view_aN(i, j, k + offs) - view_aN(i, j, k)) * dt
                                                  / hr_m[2];
                                    }
                                }
                            }
                ...
                else if (bconds_m[_d] == FDTDBoundaryCondition::ABC_FALLAHI) {
                    Kokkos::parallel_for(
                        "Scalar potential ABCs", ippl::getRangePolicy(view_phiN, 0),
                        KOKKOS_LAMBDA(const size_t i, const size_t j, const size_t k) {
                            // global indices
                            ...
                            // for (unsigned _d = 0; _d < Dim; _d++)
                            {
                                if (+bocc[_d]) {
                                    if (_d == 0) {
                                        view_phiNp1(i, j, k) = abc_x[bocc[_d] >> 1](
                                            view_phiN, view_phiNm1, view_phiNp1,
                                            ippl::Vector<size_t, 3>({i, j, k}));
                                    }
                                    if (_d == 1) {
                                        view_phiNp1(i, j, k) = abc_y[bocc[_d] >> 1](
                                            view_phiN, view_phiNm1, view_phiNp1,
                                            ippl::Vector<size_t, 3>({i, j, k}));
                                    }
                                    if (_d == 2) {
                                        view_phiNp1(i, j, k) = abc_z[bocc[_d] >> 1](
                                            view_phiN, view_phiNm1, view_phiNp1,
                                            ippl::Vector<size_t, 3>({i, j, k}));
                                    }
                                }
                    ...
                    Kokkos::parallel_for(
                        ...
                        KOKKOS_LAMBDA(const size_t i, const size_t j, const size_t k) {
                            ...
                            for (unsigned _d = 0; _d < Dim; _d++) {
                                if (+bocc[_d]) {
                                    if (_d == 0) {
                                        view_aNp1(i, j, k) = abc_x[bocc[_d] >> 1](
                                            view_aN, view_aNm1, view_aNp1,
                                            ippl::Vector<size_t, 3>({i, j, k}));
                                    }
                                    if (_d == 1) {
                                        view_aNp1(i, j, k) = abc_y[bocc[_d] >> 1](
                                            view_aN, view_aNm1, view_aNp1,
                                            ippl::Vector<size_t, 3>({i, j, k}));
                                    }
                                    if (_d == 2) {
                                        view_aNp1(i, j, k) = abc_z[bocc[_d] >> 1](
                                            view_aN, view_aNm1, view_aNp1,
                                            ippl::Vector<size_t, 3>({i, j, k}));
                                    }
                                }
        ...
        field_evaluation();
        if (field_update_m == FDTDFieldUpdateRule::DO) {
        } else if (field_update_m == FDTDFieldUpdateRule::DONT) {
            *En_mp = 0.0;
            *Bn_mp = 0.0;
        }
        ...
        Kokkos::View<Vector_t*> particleRm1_backup("particleRm1_backup", bunch.getLocalNum());
        {
            ...
            Kokkos::parallel_for(ippl::getRangePolicy(particleR_backup), KOKKOS_LAMBDA(size_t idx){
                ...
                particleRm1_backup(idx) = rm1view(idx);
            });
        }
            ...
            case FDTDParticleUpdateRule::DIPOLE_ORBIT: {
                ...
                Kokkos::parallel_for(
                    ...
                    KOKKOS_LAMBDA(const size_t i) {
                        using Kokkos::sqrt;
                        rnp1view(i)    = rview(i);
                        rnp1view(i)[1] = rview(i)[1] + bunch_dt * yvel;
                        // if(_iter == 0){
                ...
                case FDTDParticleUpdateRule::CIRCULAR_ORBIT: {
                    ...
                    Kokkos::parallel_for(
                        ...
                        KOKKOS_LAMBDA(const size_t i) {
                            ...
                            Vector_t offset_rotated =
                                Vector_t{offset[0] * cosangle - offset[1] * sinangle,
                                         offset[0] * sinangle + offset[1] * cosangle, offset[2]};
        ...
        
        {
            ...
            //R_{m}
            //particleR_backup already declared

            //R_{m - 1}
            //particleRm1_backup already declared
            
            auto phalf_view = bunch.R_np12.getView();
            auto mhalf_view = bunch.R_nm12.getView();
            Kokkos::parallel_for(ippl::getRangePolicy(particleR_backup), KOKKOS_LAMBDA(size_t idx){
                phalf_view(idx) = (rview(idx) + particleR_backup(idx)) * 0.5;
                mhalf_view(idx) = (particleRm1_backup(idx) + particleR_backup(idx)) * 0.5;
            });
            ...
            Kokkos::deep_copy(bunch.R_nm1.getView(), particleR_backup);
        }
        ...
        // Kokkos::parallel_for(
        //    bunch.getLocalNum(),
        //    KOKKOS_LAMBDA(size_t idx) { diffview(idx) = rnm1view(idx) - rview(idx); });
        // pl.update(bunch);
        // Kokkos::parallel_for(
        //    bunch.getLocalNum(),
        //    KOKKOS_LAMBDA(size_t idx) { rnm1view(idx) = diffview(idx) + rview(idx); });
        // Kokkos::parallel_reduce(Kokkos::RangePolicy<typename
        // playout_type::RegionLayout_t::view_type::execution_space>(0, bunch.getLocalNum()),
    ...
    template <typename Tfields, unsigned Dim, class M, class C>
    void FDTDSolver<Tfields, Dim, M, C>::setBoundaryConditions(
        const ippl::Vector<FDTDBoundaryCondition, Dim>& bcs) {
        typename Field_t::BConds_t scalar_bcs;
        typename VField_t::BConds_t vector_bcs;
    ...
    double FDTDSolver<Tfields, Dim, M, C>::field_evaluation() {
        ...
        Tfields global_radiation = 0.0;
        // std::cout << std::format("Rank: {}, local_radiation: {}\n", ippl::Comm->rank(),
        // radiation_on_boundary);
        MPI_Reduce(&radiation_on_boundary, &global_radiation, 1, MPI_DOUBLE, MPI_SUM, 0,
                   ippl::Comm->getCommunicator());
        global_radiation *= 4.0 * M_PI * 0.3 * 0.3 / tracer_bunch.getTotalNum();
        absorbed__energy += dt * global_radiation;
        auto lboost = this->frameBoost;
        Vector_t frame_rad_local(0.0);
        Vector_t frame_rad_global(0.0);
        const size_t slice = Eview.extent(2) - 2;
        Kokkos::parallel_reduce(ippl::getRangePolicy(En_mp->getView(), 1), KOKKOS_LAMBDA(size_t i, size_t j, size_t k, Vector_t& ref){
            if(k == slice){
                Vector_t E = Eview(i, j, k);
                Vector_t B = Bview(i, j, k);
                Kokkos::pair<Vector_t, Vector_t> pear;
                pear.first =  E;
                pear.second = B;
                Kokkos::pair<Vector_t, Vector_t> labpear = lboost.inverse_transform_EB(pear);
                Vector_t labrad = cross_prod(labpear.first, labpear.second);
                ref += labrad;
            }
        }, frame_rad_local);
        ...
        if (ippl::Comm->rank() == 0
            ...
            *(output_stream.at(trackableOutput::cumulativeRadiation))
                << this->dt * iteration << " " << absorbed__energy << std::endl;  // Flush
        }
        {
            Vector_t Jsum = JN_mp->getVolumeIntegral();
            Tfields Qsum  = rhoN_mp->getVolumeIntegral();
            if (ippl::Comm->rank() == 0) {
                //LOG("Total current: " << Jsum);
                //LOG("Total charge: " << Qsum);
            }
            // LOG("Total current: " << std::sqrt(dot_prod(JN_mp->getVolumeAverage(),
            // JN_mp->getVolumeAverage()))); auto current = JN_mp->getVolumeAverage(); LOG("Total
            // current: " << std::sqrt(dot_prod(current, current)));
        }
    ...
    void FDTDSolver<Tfields, Dim, M, C>::initialize() {
        ...
        tracer_bunch.create(tracer_count * (ippl::Comm->rank() == 0));
        auto tbrv = tracer_bunch.R.getView();
        auto tbnv = tracer_bunch.outward_normal.getView();
        Kokkos::Array<long int, 2> begin, end;
        begin[0]                 = 0;
        begin[1]                 = 0;
        end[0]                   = std::sqrt(tracer_count) * (ippl::Comm->rank() == 0);
        end[1]                   = std::sqrt(tracer_count) * (ippl::Comm->rank() == 0);
        ...
        Kokkos::parallel_for(
            ippl::createRangePolicy(begin, end), KOKKOS_LAMBDA(size_t i, size_t j) {
                ...
                ippl::Vector<Tfields, 3> offs{cos(phi) * sin(theta), sin(phi) * sin(theta),
                                              cos(theta)};
