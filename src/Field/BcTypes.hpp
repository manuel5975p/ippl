...
namespace ippl {
    ...
    template <typename Field>
    void MurABC1st<Field>::apply(Field& field) {
        // We only support constant extrapolation for the moment, other
        // higher order extrapolation stuffs need to be added.
        auto array_creator = [](unsigned i) -> std::array<unsigned, Dim - 1> {
            std::array<unsigned, Dim - 1> arr;
            unsigned index = 0;
        ...
        /*struct first_order_abc{
            unsigned main_axis = 0;
            unsigned side_axes[] = {1, 2};
            ...
            auto operator()(const auto& a_n, const auto& a_nm1,const auto& a_np1, const auto& c)const -> typename view_type::value_type{
        ...
        std::array<unsigned, Dim - 1> side_axes = array_creator(d);
        unsigned main_axis = d;
        int sign = (face & 1) ? -1 : 1;
        scalar_type beta0;
        scalar_type beta1;
        scalar_type beta2;
        scalar_type beta3;
        scalar_type beta4;
        ippl::Vector<int, Dim> offset(0);
        offset[main_axis] = sign;
        beta0 = (c * dt - hr_m[main_axis]) / (c * dt + hr_m[main_axis]);
        beta1 = 2.0 * hr_m[main_axis] / (c * dt + hr_m[main_axis]);
        beta2 = -1.0;
        beta3 = beta1;
        ...
        T ret = beta0 * (apply_with_offset<view_type, Coords, ~0u>(a_nm1, c, sign) + apply_with_offset<view_type, Coords, main_axis>(a_np1, c, sign))
                            + beta1 * (apply_with_offset<view_type, Coords, ~0u>(a_n, c, sign) + apply_with_offset<view_type, Coords, main_axis>(a_n, c, sign))
                            + beta2 * (apply_with_offset<view_type, Coords, main_axis>(a_nm1, c, sign));
                return ret;
