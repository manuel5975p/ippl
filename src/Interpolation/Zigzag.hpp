namespace ippl {
    namespace detail{
        ...
        KOKKOS_INLINE_FUNCTION constexpr int ZigzagScatterToPoint(
            ...
            const Vector<T, Dim>&, const NDIndex<Dim>, int,
            const Vector<T, Dim>&) {
            bool isinbound = true;
        ...
        KOKKOS_INLINE_FUNCTION void ZigzagScatterToField(
            ...
            auto dummy_ = (ZigzagScatterToPoint<ScatterPoint>(std::make_index_sequence<Dim>{}, view, wlo,
                                                         whi, fromi_local, jcfrom, scale, hr, lDom,
                                                         nghost, source1)
                      ^ ...);
            ...
            dummy_ = (ZigzagScatterToPoint<ScatterPoint>(std::make_index_sequence<Dim>{}, view,
                                                          wlo, whi, toi_local, jcto, scale, hr,
                                                          lDom, nghost, source2)
                       ^ ...);
