    ...
    ippl::initialize(argc, argv);
    {
        ...
        for(size_t i = 0;i < timesteps_required;i++){
            ...
            if((cfg.output_rhythm != 0) && (i % cfg.output_rhythm == 0)){
                std::cout << "Extenz: " << cfg.extents << "\n";
                rm::Vector<float, 3> pos{(float)-cfg.extents[0],0, (float)cfg.extents[2] * 0.6f};
                rm::Vector<float, 3> look_target{0,0,(float)cfg.extents[2] / 3.0f};

                rm::camera cam(pos, look_target - pos);

                ippl::Image oimg = ippl::draw_scalar_field(fdtd_state.EB, 1920, 1080, cam);
                char buf_[1024] = {0};
                snprintf(buf_, 1024, "renderdata/out%05d.bmp", (int)i);
                oimg.save_to(buf_);
                if(false){
                int img_height = 400;
